...
import { LanguageProvider } from '../../stickercraft/contexts/LanguageContext';
import { useStickerCraftRuntime } from '../../stickercraft/hooks/useStickerCraftRuntime';
import { onePaperStickerCraftPersistence } from '../services/stickerCraftGalleryService';
import type { Language } from '../../stickercraft/types';
...
const MainApp: React.FC<Props> = ({ projectId }) => {
    ...
    const isStickerWorkspaceActive = state.activeRole === 'sticker-design';
    const stickerLanguage: Language = state.lang === 'zh' ? 'zh' : 'en';
    const stickerRuntime = useStickerCraftRuntime({
        genericErrorText: state.lang === 'zh' ? '贴纸生成失败，请稍后再试。' : 'Sticker generation failed. Please try again.',
        persistence: onePaperStickerCraftPersistence,
    });
    ...
    return (
        <div className={`w-full h-screen flex flex-col overflow-hidden ${state.theme}`}>
            ...
            <LanguageProvider initialLanguage={stickerLanguage}>
            <div className="flex-1 flex overflow-hidden">
                ...
                <div id="main-canvas-area" className="flex-1 bg-stone-50 dark:bg-stone-950 relative flex flex-col min-w-0">
                    {isStickerWorkspaceActive ? (
                        <StickerWorkspace runtime={stickerRuntime} />
                    ) : (
                        <CanvasBoard
