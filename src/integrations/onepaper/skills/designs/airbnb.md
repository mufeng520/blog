...
Airbnb's 2026 design feels like a travel magazine that happens to be an app â€” pristine white canvases give way to full-bleed photography, and the interface itself disappears so the listings can breathe. The signature Rausch coral-pink (`#ff385c`) is used sparingly but unmistakably: search CTA, active tab indicator, primary action button, the occasional price or wishlist heart. Everything else is a disciplined grayscale, with `#222222` carrying almost every line of text.

What makes the system unmistakably Airbnb is how much *faith* it places in content. Property photos are displayed at hero scale, 4:3 with edge-to-edge radius treatment. Category switching happens through a tri-tab picker (Homes / Experiences / Services) that uses 3D rendered illustrated icons (a pitched-roof house, a hot-air balloon, a service bell) â€” physical, tactile, almost toy-like â€” paired with crisp `Airbnb Cereal VF` labels. This is the rare consumer product where 3D renders and purely typographic UI coexist without tension.

The newest surface is the **Experiences** product line â€” same chrome, but richer card density, more photography, and a center-anchored booking panel with sticky right-rail pricing. Listing detail pages (both rooms and experiences) follow a tight template: full-bleed hero image grid â†’ overlapping rounded booking card (sticky on scroll) â†’ amenities â†’ reviews (Guest Favorite awards use a big centered `4.81` rating with a laurel-wreath lockup) â†’ map â†’ host profile â†’ disclosures. The rhythm is consistent whether you're booking a room or a yacht tour.

**Key Characteristics:**
- Rausch coral-pink (`#ff385c`) as a single-accent brand color, used only for primary CTAs and the search button
- Full-bleed photography at 4:3 / 16:9 with gentle corner rounding (14â€“20px) as the primary visual vocabulary
- 3D rendered category icons paired with typographic tabs â€” the one place the system allows illustration
- Circular `50%` icon buttons (back arrow, share, favorite, carousel arrows) scattered throughout
- `Airbnb Cereal VF` carries every label, from 8px legal footnote to 28px section heading â€” a single-family system
- Product-tier color coding: Airbnb Plus (magenta `#92174d`), Airbnb Luxe (deep purple `#460479`), Airbnb (Rausch coral)
- Guest Favorite award lockup â€” centered giant rating number between two laurel wreaths, one of the most recognizable moments in the system
- Sticky booking panel with a price â†’ dates â†’ guests stack, pinned to the right rail on desktop, transforming to a bottom-anchored "Reserve" bar on mobile
...
- **Plus Magenta** (`#92174d`): CSS variable `--palette-bg-primary-plus`. The brand color for the Airbnb Plus product tier â€” a higher-end curated-listing offering.
- **Luxe Purple** (`#460479`): CSS variable `--palette-bg-primary-luxe`. The brand color for the Airbnb Luxe product tier â€” villa/estate-level rentals.
- **Info Blue** (`#428bff`): CSS variable `--palette-text-legal`. Used for legal/informational links (terms, privacy, disclosures) â€” the only non-monochrome link color in the system.
...
- **Hairline Gray** (`#dddddd`): Ubiquitous 1px border color â€” separates cards, amenity rows, review panels, footer columns. The workhorse of the layout system.
...
This coral â†’ magenta sweep is the "branded moment" â€” never used as a full surface, only as a narrow pill fill or logo treatment.
...
Weights observed in the extracted tokens: 500, 600, 700. No 400-regular â€” the system's "body" weight is 500, which gives every block of text a subtle extra density that reads as confident and deliberate.

OpenType features: `salt` (stylistic alternates) is used on the compact 11px and 14px 600-weight labels â€” likely for tighter numerals and special-character shaping. No ligature or fractional-numeral features observed.

### Hierarchy

| Role | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|--------|-------------|----------------|-------|
| Section Heading | 28px / 1.75rem | 700 | 1.43 | 0 | "Inspiration for future getaways" â€” page-level headings |
| Subsection Heading | 22px / 1.38rem | 500 | 1.18 | -0.44px | "What this place offers", "Meet the hosts" â€” content dividers |
| Card Title | 21px / 1.31rem | 700 | 1.43 | 0 | Review panel headings, card lead titles |
| Listing Title | 20px / 1.25rem | 600 | 1.20 | -0.18px | "Small Group Yacht Tour, Unlimited Wine & Fruits" â€” listing headlines on detail pages |
...
| Caption Bold | 14px / 0.88rem | 600 | 1.43 | 0 | `salt` feature enabled â€” numeric stats, small-text emphasis |
...
| Badge Uppercase | 11px / 0.69rem | 600 | 1.18 | 0 | `salt` feature â€” compact category/status badges |
| Superscript | 8px / 0.50rem | 700 | 1.25 | 0.32px | Uppercase â€” price footnotes, decimal tails |

### Principles
- **One family, many weights.** Airbnb Cereal VF handles everything from 8px legal to 28px page headings â€” the visual identity comes from the family itself, not from typeface mixing.
...
- **Tight line-heights for headlines, generous for body.** Display type runs at 1.18â€“1.25 (tight); body and caption open up to 1.43 for long-form comfort.
- **No all-caps except at 8px.** The only uppercase transform in the system is the 8px superscript â€” everywhere else, sentence case with subtle weight shifts does the work.
...
- Background: `#ffffff`
- Text: Ink Black `#222222`, Airbnb Cereal 500, 14â€“16px
- Padding: 10px 16px
...
- Icon: `#222222` outline stroke, 16â€“20px
- Size: 32â€“44px diameter
- Radius: 50%
...
- Paired with a 36â€“48px 3D-rendered illustrated icon above the label
...
- Shadow: none â€” separation comes from whitespace and the intrinsic radius of the photograph
...
- Radius: 14â€“20px
- Border: 1px solid Hairline Gray `#dddddd`
- Shadow: `rgba(0, 0, 0, 0.02) 0 0 0 1px, rgba(0, 0, 0, 0.04) 0 2px 6px 0, rgba(0, 0, 0, 0.1) 0 4px 8px 0` â€” a stacked three-layer subtle elevation
- Padding: 24px
- Width: ~370px, pinned 120â€“140px below the viewport top
- Content: price headline â†’ date picker â†’ guest dropdown â†’ primary CTA â†’ "You won't be charged yet" footnote
...
- Shadow: `rgba(0, 0, 0, 0.04) 0 2px 6px 0` â€” subtle floating feel
...
**Date Picker**
- Calendar grid: 7-column layout, circular `50%` day cells 40â€“44px wide
...
- Height: ~80px
- Background: `#ffffff`
- Left: Airbnb wordmark+logo lockup in Rausch (102Ã—32px)
- Center: tri-tab category picker (Homes / Experiences / Services) with 36â€“48px 3D icons stacked above 16px 500 labels; active tab has a 2px Ink Black underline
...
- Below: tri-tab category picker persists (Homes / Experiences / Services) â€” illustrated icons shrink to ~28px
- Bottom-fixed tab bar: Explore (active state Rausch) / Wishlists / Log in â€” 24px icons above 12px labels

**Listing Detail Secondary Nav**
- Sticky horizontal scroll of anchor links (Photos Â· Amenities Â· Reviews Â· Location Â· Host) appears on scroll past the hero image
- Height: 56px
...
- **Image grid on detail pages**: five-photo grid with a single large-left image (50% width) and four smaller photos in a 2Ã—2 grid on the right, all sharing the 20px outer rounded container
...
- Centered rating number rendered at 44â€“56px 700-weight
...
- Full-width block, no container border â€” sits directly on white canvas
...
- No images â€” text-only grid
- Tabbed above by category (Popular / Arts & culture / Beach / Mountains / Outdoors / Things to do / Travel tips & inspiration / Airbnb-friendly apartments) â€” active tab has 2px underline and weight shift
...
- Always shows: price headline â†’ date display â†’ guest selector â†’ Rausch CTA â†’ "You won't be charged yet" disclaimer
...
### Spacing System
- **Base unit**: 8px
- **Extracted scale**: 2, 3, 4, 5.5, 6, 8, 10, 11, 12, 15, 16, 18.5, 22, 24, 32px â€” fine-grained with a handful of off-grid values used for pixel-perfect icon alignment
- **Section padding**: ~48â€“64px top/bottom on desktop, 24â€“32px on mobile
...
- **Between stacked text rows**: 4â€“8px (very tight â€” reinforces the "dense information" feel of travel listings)

### Grid & Container
- **Max content width**: 1760â€“1920px on ultra-wide (Airbnb lets the grid breathe farther than most sites); 1280px on most detail pages
- **Homepage listing grid**: 6 columns at â‰¥1760px, 5 at â‰¥1440px, 4 at â‰¥1128px, 3 at â‰¥800px, 2 at â‰¥550px, 1 below
- **Detail page**: 2-column asymmetric â€” main content ~58%, sticky booking panel ~36% on the right, ~6% gutter
- **Footer**: 3-column Support / Hosting / Airbnb

### Whitespace Philosophy
Airbnb is densely informative but never cramped. Whitespace is used to *group* â€” listing cards have 24px of gutter so each photograph reads as a distinct object, but the metadata under each card uses 4â€“8px gaps so the price/city/date feels like a single unit. The detail-page booking panel has 24px internal padding, but rows within (date picker, guest selector, CTA) are stacked at 12px â€” the boundary between the card and the page does more separation work than the content within.
...
| 50% | All circular icon buttons, all avatars, wishlist hearts â€” the system's signature round geometry |
...
| 1 | `rgba(0, 0, 0, 0.08) 0 4px 12px` | Active/pressed icon buttons (e.g., back, share, favorite) â€” subtle lift to indicate interaction |
| 2 | `rgba(0, 0, 0, 0.02) 0 0 0 1px, rgba(0, 0, 0, 0.04) 0 2px 6px 0, rgba(0, 0, 0, 0.1) 0 4px 8px 0` | Booking panel sticky card, modals, dropdown menus â€” the system's signature three-layer elevation |
| Focus Ring | `0 0 0 2px #222222` | Active-state buttons, focused search input |
| White Separator Ring | `rgb(255, 255, 255) 0 0 0 4px` | Circular buttons overlaid on photographs â€” a 4px white ring cleanly separates the button from colorful image backgrounds |

Shadow philosophy: Airbnb uses **stacked layered shadows** rather than a single drop. The three-layer booking-panel shadow reads as one cohesive lift but is actually three separate shadows at different opacity/blur values â€” creating subtle anti-aliasing at the shadow's perimeter that feels premium without being heavy.
...
- **3D rendered category icons**: Homes/Experiences/Services icons have their own soft internal lighting and subtle cast shadows baked into the artwork â€” the only place the brand allows "dimensional" illustration

## 7. Do's and Don'ts

### Do
- Reserve Rausch `#ff385c` for primary actions and the active-tab indicator â€” never dilute it with decorative uses.
- Let photography breathe â€” 4:3 crops with 14â€“20px rounded corners, no overlaid text, no gradient scrims.
- Use Ink Black `#222222` for every text layer below Rausch â€” this is the system's near-black, never true `#000000`.
- Pair the tri-tab category picker's 3D illustrated icons with flat typography â€” don't mix illustration styles within a single surface.
...
- Use 4â€“8px spacing within metadata groups and 24px between cards â€” information density is intentional.

### Don't
- Don't introduce secondary accent colors outside the Rausch / Plus Magenta / Luxe Purple product-tier palette.
- Don't place text inside photographs â€” captions always sit below the image, never overlaid.
- Don't use all-caps labels except the single 8px superscript role.
- Don't round icon buttons to anything other than 50% â€” circular is the system's signature geometry.
- Don't add drop shadows to listing cards â€” they sit on white canvas with no elevation.
- Don't use gradient backgrounds â€” the only gradient in the system is a narrow Rausch â†’ magenta sweep on the wordmark.
- Don't use the 400-regular font weight â€” Airbnb Cereal's body weight is 500.
- Don't override Airbnb Cereal VF with a different display face â€” the system is intentionally single-family.
...
|------|-------|-------------|
| Ultra-wide | â‰¥1760px | 6-column listing grid, 1760â€“1920px max content width |
| Desktop XL | 1440â€“1759px | 5-column grid, full nav visible, sticky right-rail booking panel |
| Desktop | 1128â€“1439px | 4-column grid, sticky booking panel persists |
| Laptop | 1024â€“1127px | 3â€“4 column grid, category nav remains horizontal |
| Tablet | 800â€“1023px | 3-column grid, global search may collapse to a single-row pill |
| Small tablet | 550â€“799px | 2-column grid, booking panel drops to full-width inline block |
| Mobile | 375â€“549px | 1-column stacked layout, bottom-fixed tab bar appears (Explore / Wishlists / Log in) |
| Small mobile | <375px | Edge padding tightens to 16px; category-picker icons shrink to ~28px |

### Touch Targets
All interactive elements meet or exceed 44Ã—44px. The circular icon button family is specifically sized 32â€“44px with 8â€“12px extended hit-area padding. The Rausch primary Reserve button is ~48px tall. The tri-tab category picker's hit area is the full label-plus-icon rectangle (typically ~64Ã—80px per tab).
...
- **Booking panel**: Sticky right-rail on â‰¥1128px; inline within the main content column between 800â€“1127px; bottom-fixed "Reserve" pill on <800px.
- **Listing grid**: Reflows 6 â†’ 5 â†’ 4 â†’ 3 â†’ 2 â†’ 1 columns across breakpoints.
...
- No art-direction crops â€” the same image is scaled up/down across breakpoints
...
- "Create a primary Reserve button: Rausch (#ff385c) background, white Airbnb Cereal 500-weight label at 16px, 14px Ã— 24px padding, 8px border-radius, no shadow. On active/pressed add `transform: scale(0.92)` with a 2px Ink Black focus ring (`0 0 0 2px #222222`)."
...
5. Always default to Airbnb Cereal VF 500-weight for body and 600â€“700 for emphasis â€” never 400.
6. Keep Rausch pink scarce â€” if more than one Rausch-colored element appears per viewport, consider whether one should be neutralized.

### Known Gaps
- **Homepage listing grid cards**: the main property-card grid (the primary visual surface of airbnb.com) was not fully captured in the extracted homepage screenshots â€” content loaded only partially. Listing Card specs above are inferred from the Inspiration grid structure and Airbnb's broader conventions; confirm exact aspect ratios and metadata hierarchy against the live site before production use.
...
- **Animation and transition timings**: not captured â€” static extraction scope.
