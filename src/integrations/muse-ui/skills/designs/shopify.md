...
- Zinc-based neutral scale for text hierarchy â€” balanced between warm and cool
...
- **Neon Green** (`#36F4A4`): The signature accent â€” focus rings, interactive highlights, active state indicators. Electric and bioluminescent
...
- **Void** (`#000000`): Root page background â€” true black for maximum depth
- **Deep Teal** (`#02090A`): Card surfaces, content containers â€” near-black with green undertone
- **Dark Forest** (`#061A1C`): Section backgrounds with visible green character
- **Forest** (`#102620`): Elevated dark surfaces, header backgrounds â€” the warmest dark shade
...
- **Muted Text** (`#A1A1AA`): Secondary text, metadata, descriptions â€” the quiet voice
...
- **Light Border** (`#E4E4E7`): Borders on light surfaces (rare â€” only in light-mode modals)
...
- **Dark Teal Wash**: Radial gradient from `#102620` center to `#02090A` edge â€” used behind product showcases
- **Green Atmospheric**: Subtle green-tinted ambient gradients behind hero sections, creating depth without solid colors
- **Spotlight**: Focused bright area fading to black â€” creates keynote-style presentation lighting
...
- OpenType features: `ss03` (stylistic set 3 â€” distinctive letterform alternates)
...
|------|------|--------|-------------|----------------|-------|
| Display XL | 96px | 400 | 1.00 | â€” | NeueHaasGrotesk, hero headlines, "ss03" |
...
| Display Light | 96px | 330 | 0.96 | â€” | NeueHaasGrotesk, ethereal display |
| Heading 1 | 70px | 330 | 1.00 | â€” | NeueHaasGrotesk, section titles |
| Heading 2 | 55px | 330 | 1.16 | â€” | NeueHaasGrotesk, subsections |
| Heading 3 | 48px | 330 | 1.14 | â€” | NeueHaasGrotesk, feature titles |
...
| Body | 18px | 400 | 1.56 | â€” | Inter-Variable, standard body |
| Body Medium | 18px | 550 | 1.56 | â€” | Inter-Variable, emphasized body |
| Body Small | 16px | 400 | 1.50 | â€” | Inter / NeueHaasGrotesk, compact body |
| Body Small Medium | 16px | 420 | 1.50 | â€” | Inter-Variable, slightly emphasized |
| Button | 16px | 400 | 1.50 | â€” | NeueHaasGrotesk, CTA text |
...
- Padding: 12px 26px 12px 16px (asymmetric â€” more right padding for visual balance)
...
- Border: 1px solid `#1E2C31` (Dark Card Border) â€” barely visible boundary
...
- Aspect ratios: varied â€” hero images are wide (16:9-ish), feature shots are flexible
- All images sit flush within dark containers â€” no bright borders or frames
...
Shopify's whitespace strategy is theatrical. Sections are separated by vast expanses of dark space â€” 80px to 120px of pure black breathing room â€” that create the pacing of a presentation, not a webpage. Each content block is its own "slide" in a keynote-style scroll. Within sections, spacing is tighter and more deliberate, creating focal density against the expansive void. The contrast between macro-level emptiness and micro-level precision is what gives the site its cinematic cadence.
...
- Use the dark teal-black surface hierarchy (Void â†’ Deep Teal â†’ Dark Forest â†’ Forest) for depth
- Keep display typography at weight 330-400 â€” the ethereal lightness is the design's signature
- Use Neon Green (`#36F4A4`) exclusively for focus states and critical accent highlights
- Apply 9999px radius to all primary CTA buttons â€” the full pill is non-negotiable
- Use the multi-layered shadow system for card elevation â€” single shadows look flat
- Maintain the `ss03` OpenType feature across all text â€” it's part of the typographic identity
- Use Inter Variable for body text and NeueHaasGrotesk for headings â€” never mix their roles
- Create theatrical spacing between sections (80px+) for cinematic pacing

### Don't

- Don't use pure black (#000000) for text on dark backgrounds â€” use white (#FFFFFF) only
- Don't introduce warm colors (orange, red, yellow) â€” the palette is strictly cool (greens, teals, neutrals)
- Don't use font weights above 500 for NeueHaasGrotesk body text â€” heavy weights break the ethereal feel
- Don't apply green accents to large surfaces â€” Neon Green is for small, precise highlights only
- Don't use sharp corners (0px radius) on interactive elements â€” everything rounds
- Don't add bright backgrounds â€” the dark theme is fundamental, not optional
- Don't use single-layer box shadows â€” the stacked approach is the system
- Don't set line-height above 1.56 for body text â€” Shopify's text is relatively compact
- Don't mix NeueHaasGrotesk and Inter at the same size/role â€” their weight scales differ
- Don't use letter-spacing below 0 for headings â€” Shopify headings track neutral or positive
...
- **Navigation**: Full horizontal links â†’ hamburger menu below 1024px; logo and CTA button remain visible
- **Hero section**: 96px display â†’ 70px at tablet â†’ 48px on mobile; maintains single-column center alignment
- **Feature sections**: 2-column text+image â†’ stacked single column below 768px
- **Stats**: Horizontal row â†’ stacked vertical on mobile
- **Section padding**: 64px â†’ 40px â†’ 24px â†’ 16px as viewport narrows
- **Cards**: Grid â†’ stack, maintaining full-width on mobile
...
- "Design a tag/badge with rgba(255,255,255,0.2) frosted glass background, 4px radius, 12px 16px padding, white 16px text â€” floating over a dark card surface"
...
3. Remember: this is a DARK-FIRST design â€” light surfaces are the exception, not the rule
4. Display text should always feel feather-light (weight 330-400) â€” if it looks heavy, reduce the weight
5. Neon Green (#36F4A4) is precious â€” use sparingly for focus and accent only
6. The dark surface hierarchy (black â†’ deep teal â†’ dark forest â†’ forest) creates subtle depth
7. Shadows are multi-layered â€” a single `box-shadow` value won't capture the Shopify card feel
