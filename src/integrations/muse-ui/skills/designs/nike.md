...
- Full-bleed photography with no border radius â€” imagery fills every available edge
- Pill-shaped buttons (30px radius) as the primary interactive element
- 8px spacing grid with athletic discipline â€” every measurement snaps to the system
- Category-driven shopping architecture with large navigational image cards
- Shadow-free, border-minimal elevation model â€” surface differentiation through grey shifts only

## 2. Color Palette & Roles

### Primary

- **Nike Black** (`#111111`): The foundation â€” primary text, button backgrounds, nav text, hero overlays. Deliberately not pure black (#000000), creating a fractionally softer reading experience
...
Each color ramp runs 50â€“900 for expressive use in campaigns and product pages:

- **Red**: `#FFE5E5` â†’ `#EE0005` â†’ `#530300`
- **Orange**: `#FFE2D6` â†’ `#FF5000` â†’ `#3E1009`
- **Yellow**: `#FEF087` â†’ `#FCA600` â†’ `#99470A`
- **Green**: `#DFFFB9` â†’ `#1EAA52` â†’ `#003C2A`
- **Teal**: `#D4FFFB` â†’ `#008E98` â†’ `#043441`
- **Blue**: `#D6EEFF` â†’ `#1151FF` â†’ `#020664`
- **Purple**: `#E4E1FC` â†’ `#6E0FF6` â†’ `#1C0060`
- **Pink**: `#FFE1F3` â†’ `#ED1AA0` â†’ `#4C012D`

### Gradient System

Nike avoids UI gradients. When gradients appear, they are photographic â€” applied to product hero backgrounds (e.g., a red shoe on a red-to-deeper-red gradient). The design system itself is flat-color only.
...
- Used for section headings and product titles at 24â€“32px
...
**Arabic:** Neue Frutiger Arabic â€” locale-specific alternative

### Hierarchy

| Role | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|--------|-------------|----------------|-------|
| Display | 96px | 500 | 0.90 | â€” | Nike Futura ND, uppercase, hero headlines |
| Heading 1 | 32px | 500 | 1.20 | â€” | Helvetica Now Display Medium, section titles |
| Heading 2 | 24px | 500 | 1.20 | â€” | Helvetica Now Display Medium, subsections |
| Heading 3 | 16px | 500 | 1.50 | â€” | Helvetica Now Text Medium, card titles |
| Body | 16px | 400 | 1.75 | â€” | Helvetica Now Text, product descriptions |
| Body Medium | 16px | 500 | 1.75 | â€” | Helvetica Now Text Medium, emphasized text |
| Link | 16px | 500 | 1.75 | â€” | Helvetica Now Text Medium, navigation links |
| Link Small | 14px | 500 | 1.86 | â€” | Helvetica Now Text Medium, footer/utility links |
| Button | 16px | 500 | 1.50 | â€” | Helvetica Now Text Medium, CTA text |
| Button Small | 14px | 500 | 1.50 | â€” | Helvetica Now Text Medium, secondary buttons |
| Caption | 14px | 500 | 1.50 | â€” | Helvetica Now Text Medium, price labels |
| Small | 12px | 500 | 1.50 | â€” | Helvetica Now Text Medium, timestamps |
| Tiny | 12px | 400 | 1.50 | â€” | Helvetica Now Text, legal text |

### Principles

Nike's typography is a study in tension. The display layer â€” Nike Futura ND at 96px with a devastating 0.90 line-height â€” is engineered to feel like a stadium scoreboard: massive, condensed, uppercase, impossible to ignore. It transforms headlines into battle cries. Below the display layer, Helvetica Now provides a clinical counterpoint: Swiss-precision legibility with generous 1.75 line-height for comfortable product browsing. Weight 500 (Medium) dominates throughout the body text, giving Nike's prose a slight assertiveness without the heaviness of bold â€” every sentence reads like a confident recommendation, not a shout.
...
- Background: White (`#FFFFFF`) â€” no visible card boundary in most cases
- Border radius: 0px for product image cards (edge-to-edge imagery), 20px for interactive containers
- Shadow: none â€” Nike uses no card shadows whatsoever
...
- Product hover: secondary image swap (front â†’ side view)
...
Nike's whitespace strategy is deliberately aggressive â€” not in the luxurious, breathing way of a fashion brand, but in a compressed, high-density way that fills every pixel with either content or intentional absence. Product grids use minimal gaps (4-12px) to create a sense of abundance and choice. Section breaks are generous (48-80px) to separate shopping contexts. The overall effect is a store that feels packed with product while remaining navigable â€” like a well-organized athletic superstore.
...
- Use Nike Black (#111111) for all primary text â€” never pure #000000
- Keep buttons pill-shaped (30px radius) and limited to primary/secondary variants
- Use full-bleed, edge-to-edge photography for hero sections â€” no border radius on images
...
- Don't add shadows to cards â€” Nike's elevation model is entirely flat
- Don't use border radius on product imagery â€” only UI elements get rounded corners
- Don't introduce brand colors beyond the grey scale for UI elements
- Don't use Nike Futura ND below 24px â€” it's exclusively a display face
- Don't add hover lift effects â€” Nike cards don't animate on hover
- Don't use regular weight (400) for buttons or links â€” always use 500
- Don't place colored backgrounds behind UI elements â€” color is reserved for product contexts
- Don't use more than two levels of text hierarchy per card (title + body)
- Don't add decorative dividers â€” the 1px inset is the only divider pattern
- Don't soften the contrast â€” Nike's design deliberately pushes black-on-white to maximum
...
- **Navigation**: Full category links â†’ hamburger menu below 960px; search, favorites, cart icons remain visible
- **Product grids**: 3-col â†’ 2-col at 960px â†’ 1-col at 640px
- **Hero sections**: Display text scales from 96px â†’ 64px â†’ 48px; hero images remain full-bleed at all sizes
- **Category cards**: 3-col â†’ 2-col â†’ 1-col with maintained full-bleed imagery
- **Section padding**: 80px â†’ 48px â†’ 32px â†’ 24px as viewport narrows
...
- Hero images: full-bleed at all breakpoints, aspect ratio shifts (16:9 desktop â†’ 4:3 mobile)
...
- "Create a promotional banner strip with #111111 background, white 12px/500 centered text, and 8px vertical padding â€” full width, no border radius"
...
3. Remember: product photography is the color â€” UI stays monochromatic
4. Use the grey scale for state changes: #F5F5F5 â†’ #E5E5E5 â†’ #CACACB â†’ #707072
5. If something feels too colorful in the UI, it probably is â€” Nike keeps UI greyscale
