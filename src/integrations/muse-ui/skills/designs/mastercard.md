...
The second gesture is **orbit and trajectory**. Circular image masks don't sit still â€” they're connected by thin, hand-drawn-feeling orange arcs that span entire viewport widths, implying a constellation of services rather than a list. Each circle has a small attached "satellite" â€” a white micro-CTA holding an arrow icon â€” docked onto its perimeter like a moon. This is the most distinctive thing about Mastercard's current design language: the circles feel like they're in motion even though the page is still.

Typography is rendered entirely in **MarkForMC**, Mastercard's proprietary geometric sans. Headlines are set at a medium weight (500) with tight negative letter-spacing (-2%), giving them confidence without shouting. Body copy runs at the same family in a slightly lighter weight (450) â€” a weight you rarely see on the web, chosen because it reads softer than regular 400 without feeling thin. The whole system â€” warm cream surfaces, pill shapes, circular portraits, traced-orange orbits, black CTAs â€” feels simultaneously institutional (a 60-year-old payments network) and editorial (a modern brand magazine), which is exactly the tension Mastercard wants to hold.

**Key Characteristics:**
- Warm cream canvas (`#F3F0EE`) replaces traditional white â€” every surface is tinted, never sterile
...
- Black primary CTAs with 20px radius in the body â€” the cookie-banner orange is kept to consent flows
- Floating pill-shaped navigation that docks below the viewport top with rounded shoulders
- Eyebrow labels with a tiny accent dot + uppercase bold tracking â€” used as the section-category signal
- Dark warm-black footer (`#141413`) with four-column link layout and large conversational headline

## 2. Color Palette & Roles

### Primary
- **Mastercard Red** (`#EB001B`): The left circle of the Mastercard mark â€” used only in the brand logo, never as a UI color.
- **Mastercard Yellow** (`#F79E1B`): The right circle of the Mastercard mark â€” used only in the brand logo, never as a UI color.
...
### Secondary & Accent
- **Signal Orange** (`#CF4500`): The burnt/rust CTA orange used on consent actions and eyebrow dots. Deeper than the brand yellow, brighter than ink â€” it's the page's single aggressive color and must be used sparingly.
...
- **Lifted Cream** (`#FCFBFA`): One step lighter than canvas â€” used for nested "raised" sections that want to feel like paper laid on paper.
...
- **Slate Gray** (`#696969`): Muted secondary text â€” eyebrow label alternative, disabled states, "Privacy Choices" bottom-row text.
...
### Font Family
- **Primary**: `MarkForMC` â€” Mastercard's proprietary geometric sans. Every headline, body paragraph, button, nav link, and footer link on the page.
- **Secondary**: `MarkOffcForMC` â€” an "Official" cut used in a minority of contexts (legal text, some forms).
- **Fallback stack**: `SofiaSans, Arial, sans-serif` â€” Sofia Sans is a reasonable open-source stand-in; Arial is the final web-safe fallback.
...
| Eyebrow (H5) | 14px | 700 | 14px | 0.56px (+4%) | Uppercase, paired with a tiny accent dot (e.g., "â€¢ SERVICES") |
| Body paragraph | 16px | 450 | 22.4px (1.4) | normal | The half-step 450 weight is MarkForMC's signature â€” softer than 500, firmer than 400 |
...
| Footer column header | 12â€“14px | 700 | 14px | 0.56px (+4%) | Uppercase, muted gray, short tracking |
...
- **Tight negative tracking on headlines** (-2%) gives display text its editorial density â€” the words lock together rather than breathe.
...
- **Sofia Sans** (Google Fonts) is the closest open-source match â€” it's already in Mastercard's declared fallback stack.
...
- Text: Canvas Cream (`#F3F0EE`) â€” not pure white
...
- Use for: cookie consent, privacy preference, and other legally-distinct confirmations. **Do not** use this orange for marketing CTAs â€” it reads as a compliance color.

**Satellite â€” Circular Micro-CTA**
- Background: White (`#FFFFFF`)
- Icon: Ink Black arrow (`â†’`) at ~20px
- Border: none
- Radius: 50% (perfect circle)
- Size: ~50â€“60px diameter
...
- Height: ~60â€“70% of viewport
- Shadow: none (sits directly on canvas)
- Corners: the extreme 40px radius on a media element is the most iconic Mastercard gesture â€” do not round less

**Service / Solution Portrait Card**
- Shape: Perfect circle (radius 50%) or ellipse (radius 999px / 1000px)
- Diameter: 260â€“340px desktop; ~220px mobile
...
- Eyebrow below: accent dot + uppercase label (e.g., "â€¢ SERVICES", "â€¢ SOLUTIONS")
- Title below: H3 (24px / weight 500 / -2% tracking), 1â€“2 lines max
...
- Width: ~40â€“60% of viewport
- Height: ~380â€“420px (portrait-pill orientation)
...
- Font: MarkForMC 72â€“128px / weight 500 / tight -2% tracking
- Color: Canvas Cream slightly darkened (`#E8E2DA` or similar â€” cream-on-cream)
...
- Shadow: very soft (`rgba(0, 0, 0, 0.04) 0px 4px 24px 0px`) â€” just enough to lift it off the cream canvas
...
- Link spacing: ~48â€“56px gap between primary links
...
- **Aspect ratios used**: 1:1 (all service portraits â€” cropped to circle), ~3:4 or ~4:5 (carousel pill cards), 16:9 or wider (hero video frame)
...
- **Masking**: Aggressive circular masking is the defining treatment â€” square source images are cropped to perfect circles of matching diameter. Never use rectangular service imagery.
...
A signature motif: thin (~1â€“1.5px) single-weight curved lines in Light Signal Orange (`#F37338`) tracing arcs between circular portraits. These lines:
...
- Appear only in sections with circular portrait content â€” never on pill sections, never in the footer
...
- External link marker: a small upper-right arrow (`â†—`) after link text
...
- **Section vertical padding**: ~96â€“128px between major sections on desktop; ~48â€“64px on mobile
- **Card internal padding**: 32â€“40px on desktop, ~24px on mobile
- **Nav top margin**: ~24px from viewport top (the pill floats, doesn't touch)

### Grid & Container
- **Max content width**: ~1200â€“1280px centered, with ~48â€“100px horizontal gutter
...
- ~300â€“500px of blank canvas between the portrait and the next section
This deliberate emptiness tells the eye "slow down, read one thing at a time" â€” the opposite of dense dashboard UIs.

### Border Radius Scale

| Radius | Use |
|--------|-----|
| 3â€“6px | Tiny decorative elements, cookie banner micro-chips |
...
| 99px / 999px / 1000px | Full pill shapes â€” navigation, carousel cards, footer country selector, primary inline chips |

The scale is unusual: most systems use 4/8/12/16. Mastercard skips those and commits to **either small (â‰¤6), medium-large (20â€“40), or full-pill (99+)**. The middle ground of 8â€“12 is absent, which is why the UI feels either "precise and utility" or "soft and editorial" with no in-between.

## 6. Depth & Elevation

| Level | Treatment | Use |
|-------|-----------|-----|
| 0 | No shadow | The default â€” 95% of surfaces sit directly on cream canvas |
| 1 | `rgba(0, 0, 0, 0.04) 0px 4px 24px 0px` | Floating nav pill â€” barely-there lift |
| 2 | `rgba(0, 0, 0, 0.08) 0px 24px 48px 0px` | Hero media frames, elevated cards â€” a soft large-radius halo rather than a hard drop |
| 3 | `rgba(0, 0, 0, 0.25) 0px 70px 110px 0px` | Rare; dramatic elevation on a feature tile |

### Shadow Philosophy
Mastercard uses shadows as **atmospheric cushioning**, not directional light. The Level 2 shadow has a 48px spread and only 8% opacity â€” it barely exists as dark pixels but creates a "the card is breathing above the canvas" feel. There are almost no hard-edged, tight shadows anywhere in the system. Border lines are preferred over shadows for functional delineation (form inputs, footer divider).
...
### Do
- Use Canvas Cream (`#F3F0EE`) as the default body background â€” never pure white
...
- Build page rhythm from three surface tones: canvas cream â†’ lifted cream â†’ ink footer
- Use thin Light Signal Orange arcs between service cards to imply connection

### Don't
- Don't use pure white as a page background â€” it breaks the warm editorial tone
- Don't round image frames at 8â€“16px â€” Mastercard either uses full-pill, 40px, or full-circle. In-between radii look generic
- Don't use Signal Orange for marketing CTAs â€” it reads as cookie-consent orange and dilutes the legal color signal
- Don't mix typefaces â€” no serif accent, no script, no secondary display font
- Don't crowd the nav with more than six top-level links â€” the pill is meant to feel airy
- Don't drop hard shadows â€” all elevation should use 48px+ spread and â‰¤10% opacity
- Don't use uppercase for anything larger than the 14px eyebrow label
- Don't omit the tiny accent dot before eyebrow labels â€” it's the identity
- Don't place circular portraits on a grid â€” their magic comes from asymmetric placement
...
|------|-------|-------------|
| Mobile | â‰¤ 767px | Nav pill shows logo + menu + search only; primary links hide behind hamburger; service portraits stack single-column centered; hero headline drops from 64px to ~40px; footer columns collapse into a vertical accordion |
| Tablet | 768â€“1023px | Nav pill shows 2â€“3 primary links truncated; service portraits arrange 2-up; hero headline ~48px |
| Desktop | â‰¥ 1024px | Full nav with 5 primary links centered; service portraits asymmetrically placed with decorative orbital lines; hero headline 64px |
| Wide | â‰¥ 1440px | Content max-width caps at ~1280px; gutters grow symmetrically; orbital lines extend further |

### Touch Targets
All interactive elements comfortably exceed 44Ã—44px. The satellite CTA (circle + arrow) is ~50â€“60px. The nav pill buttons are ~48px tall. Mobile hamburger and search are 48Ã—48px. No link or button drops below 40px in any breakpoint.

### Collapsing Strategy
- **Nav**: full pill â†’ compact pill with hamburger. Pill shape is preserved across breakpoints â€” always rounded, always floating.
- **Service grid**: asymmetric constellation â†’ 2-up â†’ 1-up stack. Orbital arcs are removed on mobile (they only work with asymmetric placement).
...
- **Footer**: 4 columns â†’ 1 column accordion with chevron toggles per section.
...
- Primary CTA: "Ink Black (`#141413`) â€” the warm near-black used for primary pill buttons and footer"
- Background: "Canvas Cream (`#F3F0EE`) â€” warm putty body canvas, never pure white"
- Lifted surface: "Lifted Cream (`#FCFBFA`) â€” one step lighter than canvas for nested sections"
- Heading text: "Ink Black (`#141413`)"
- Body text: "Ink Black (`#141413`) at weight 450"
- Muted text: "Slate Gray (`#696969`)"
- Signal / Consent: "Signal Orange (`#CF4500`) â€” reserve for cookie consent and legal actions"
- Accent arc: "Light Signal Orange (`#F37338`) â€” orbital decorative lines only"
...
1. Focus on ONE component at a time â€” don't redesign multiple surfaces in parallel
...
6. Default backgrounds to Canvas Cream (`#F3F0EE`), not white â€” this single change shifts the entire mood toward Mastercard
...
- Tablet breakpoint specifics (768â€“1023px) were inferred from desktop and mobile captures; intermediate layouts may vary per section.
