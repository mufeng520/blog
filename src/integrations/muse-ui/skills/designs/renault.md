...
Renault's website is a vibrant digital showroom that balances French automotive elegance with bold, forward-leaning energy â€” a departure from the monochromatic austerity of German or Italian luxury brands. The page opens with a full-screen hero that washes the viewport in a sweeping aurora gradient â€” ribbons of magenta, violet, and teal bleeding across the frame behind a dramatically lit vehicle. This chromatic expressiveness is the site's signature: while the interface structure is disciplined (NouvelR typography, black-and-white CTA framework, zero-radius buttons), the content is alive with color â€” gradient washes on hero slides, saturated vehicle photography, and splashes of Renault Yellow (`#EFDF00`) on accent CTAs. The effect is a showroom that feels energized rather than hushed.

The layout follows a card-based editorial rhythm. Below the hero carousel, content is organized into a grid of PromoCards â€” each a full-bleed photographic panel with a dark gradient overlay at top (fading from `rgba(0,0,0,0.6)` to transparent) to ensure white heading text remains legible over vivid imagery. These cards alternate between light and dark modes: white editorial panels with black text sit beside black `is-alternative-mode` sections with white text, creating a chessboard-like visual cadence. The grid is generous â€” large card formats dominate, giving each vehicle or campaign its own visual territory. The lower sections shift to a fully dark canvas (Absolute Black backgrounds) for the E-Tech electric and technology showcases, establishing a deliberate mood shift: electrification lives in darkness, tradition in light.

Typography is unified under NouvelR â€” a proprietary geometric sans-serif designed by Black[Foundry] exclusively for Renault's rebrand. The typeface features a distinctive "radical r" with a terminal cut at 28 degrees to echo the Renault diamond logo's angles. Available in 6 weights from Light to Extrabold, the site primarily uses Bold (700) for headings and Regular (400) for body. Display headlines run large â€” 56px/0.95 line-height for hero titles, creating dense, impactful text blocks that sit tight against each other. The font supports Latin, Greek, Cyrillic, Hebrew, Arabic, and Korean, reflecting Renault's global market reach. All text rendering feels precise and engineered, with the geometric proportions lending a sense of modernity that aligns with Renault's electric-first brand positioning.
...
- Renault Yellow (`#EFDF00`) as the super-primary accent â€” used sparingly for highest-priority CTAs
- Zero border-radius on all buttons â€” sharp rectangular forms expressing precision engineering
...
- Link hover state in Renault Blue (`#1883FD`) â€” the sole chromatic interaction color

## 2. Color Palette & Roles

### Primary
- **Renault Yellow** (`#EFDF00`): The brand's signature Pantone â€” a vivid, saturated yellow used for super-primary CTAs and the highest-priority action buttons. Appears as `--CtaLink-background-color` on `.is-cta-super-primary` class. Carries the energy of the diamond logo
...
### Secondary & Accent
- **Soft Yellow** (`#F8EB4C`): Lighter, warmer variant of Renault Yellow â€” used for hover/pressed states on yellow CTAs and secondary accent contexts
- **Renault Blue** (`#1883FD`): Link hover color across all link variants â€” a bright, confident blue that signals interactivity without competing with the yellow brand accent
- **Warm Gray** (`#D9D9D6`): Subtle warm neutral used for disabled states, inactive UI elements, and soft borders â€” carries a slight warmth that distinguishes it from cold grays
...
### Neutrals & Text
- **Absolute Black** (`#000000`): Primary heading and body text on light surfaces â€” Renault uses true black rather than near-black
- **Pure White** (`#FFFFFF`): Primary text on dark surfaces â€” hero headlines, dark-section headings, and inverted button labels
...
### Gradient System
- **Hero Aurora**: Sweeping multi-color gradients (magenta â†’ violet â†’ teal) applied to hero slide backgrounds â€” the site's most distinctive visual element. These are photographic/composited rather than CSS gradients
- **PromoCard Overlay**: `linear-gradient(rgba(0,0,0,0.6) 0%, rgba(0,0,0,0) 40%)` â€” applied to card tops to ensure heading text legibility over photography
- No flat CSS gradients on surfaces â€” depth comes from photographic treatment and the black/white alternation
...
- **No secondary typeface**: Unlike Ferrari (FerrariSans + Body-Font) or Lamborghini (LamboType + Open Sans), Renault uses a single font family for all text â€” headings, body, buttons, captions, and navigation
...
### Principles
- **Single-family discipline**: NouvelR handles everything from 56px hero headlines to 8.5px legal captions â€” the font's geometric precision allows it to scale across this extreme range without losing character
...
- **Ultra-tight display line-heights**: 0.95 line-height on hero and section headings â€” the lines nearly collide, creating a compressed, punchy typographic texture that feels urgent and modern
- **28-degree radical r**: The typeface's signature detail â€” the lowercase "r" terminal is cut at precisely 28 degrees to mirror the angles of the Renault diamond logo, embedding brand identity into every word
- **Capitalize transform on captions**: Some caption text uses `text-transform: capitalize` for editorial labeling, while micro text uses `lowercase` â€” a deliberate inversion for hierarchy signaling

## 4. Component Stylings

### Buttons
Renault's buttons are sharp-edged rectangles with zero border-radius â€” the industrial precision of a pressed metal body panel.

**Super Primary (Yellow)** â€” The highest-emphasis CTA:
- Default: bg `#EFDF00` (Renault Yellow), text `#000000`, borderRadius 0px, padding 10px 15px, border 1px solid `#EFDF00`
- Inverted: bg `#EFDF00`, text `#000000` â€” same yellow on dark backgrounds
- fontSize 16px (NouvelR), fontWeight 700, minHeight 46px, minWidth 46px
- Used for: Primary conversion actions (configure, buy now)

**Primary (Black)** â€” The default action button:
- Default: bg `#000000`, text `#FFFFFF`, borderRadius 0px, padding 10px 15px, border 1px solid `#000000`
- Inverted: bg `#FFFFFF`, text `#000000`, border 1px solid `#FFFFFF` â€” white fill on dark backgrounds
- fontSize 16px (NouvelR), fontWeight 700
- Used for: "keÅŸfedin" (explore), secondary conversion actions

**Ghost** â€” Transparent outline button:
...
- fontSize 16px (NouvelR), fontWeight 700
- Used for: "ilk sen Ã¶ÄŸren" (be the first to know), "satÄ±n alÄ±n" (buy), secondary actions
...
- All link variants hover to the same blue â€” consistent interactive feedback

### Cards & Containers

**PromoCard (Light)** â€” Editorial content card:
...
- Border-radius: 0px â€” sharp rectangular containers
- No shadow, no visible border

**PromoCard (Dark / `is-alternative-mode`)** â€” Cinematic card:
...
**VehicleRangeCard** â€” Vehicle showcase:
...
- Border-radius: 50px (pill-shaped â€” unusual deviation from the zero-radius button system)
...
### Image Treatment
- **Hero**: Full-viewport carousel with dramatic aurora-gradient backgrounds and art-directed vehicle photography â€” edge-to-edge, no padding
...
- **Aspect ratios**: Mixed â€” hero at roughly 16:9 viewport, promo cards at various ratios from square to wide panoramic
...
- **Button padding**: 10px 15px â€” consistent across all button variants
- **Section padding**: Generous vertical spacing (40â€“80px) between major content blocks
- **Card gaps**: 16â€“24px between grid items
...
Renault uses whitespace moderately â€” more generously than Ferrari but less extremely than Tesla. The card-based layout means content is organized into defined containers rather than floating in void. The visual breathing room comes primarily from the large card formats and the full-bleed hero carousel, which gives each vehicle its own cinematic moment. Between sections, spacing is consistent (32â€“40px) creating a rhythmic scroll experience. The alternation between light and dark sections also creates perceived whitespace â€” the mode switch itself acts as a visual separator.

### Border Radius Scale
| Value | Context |
|-------|---------|
| 0px | All buttons, PromoCards, most containers â€” the zero-radius default |
...
### Shadow Philosophy
Renault uses a richer shadow system than Ferrari or Tesla â€” seven distinct shadow tokens reflecting a more layered, dimensional interface. The shadows progress from subtle 4px hover lifts to dramatic 80px deep panels. The compound shadow (Level 3) with its dual-layer approach (a tight dark shadow plus a wider purple-tinted one from `rgba(50,50,93,0.1)`) is particularly refined â€” it creates a photorealistic floating effect. The ambient glow (Level 6) in warm gray adds a unique touch that connects to Renault's warmer color personality.

### Decorative Depth
- **Hero aurora gradients**: The primary decorative depth element â€” vivid color gradients create atmospheric depth behind vehicle imagery
- **PromoCard overlays**: `linear-gradient(rgba(0,0,0,0.6) â†’ transparent)` creates depth within cards through transparency
- **No blur effects** on UI elements â€” depth is communicated through shadow and color contrast

## 7. Do's and Don'ts

### Do
- Use Renault Yellow (`#EFDF00`) exclusively for super-primary CTAs â€” it carries the full weight of the diamond logo's identity
- Maintain zero border-radius on all buttons â€” sharp edges are non-negotiable in the Renault system
- Use NouvelR Bold (700) as the default heading weight â€” the assertive weight is central to the brand's energetic personality
- Apply the dark gradient overlay (`rgba(0,0,0,0.6) â†’ transparent`) on PromoCards to ensure text legibility over photography
- Keep hero line-heights ultra-tight (0.95) for display text â€” the compressed texture feels urgent and modern
- Alternate between black and white sections to create the signature chessboard rhythm
- Use `#1883FD` (Renault Blue) consistently for all link hover states â€” one interactive color signal
- Set minimum interactive size at 46Ã—46px for all buttons â€” accessibility built into the component spec
- Reserve pill-shaped radius (46â€“50px) exclusively for search inputs and filter elements â€” never for buttons
- Use the PromoCard gradient overlay on every card that has text over photography

### Don't
- Apply Renault Yellow as a background color for sections or surfaces â€” it's a CTA signal, not an atmosphere color
- Add border-radius to buttons â€” the zero-radius rectangle is a core brand marker
- Use any typeface besides NouvelR â€” the single-family discipline is a brand pillar
- Mix multiple chromatic accent colors in a single section â€” the palette is monochrome-plus-yellow
- Soften heading weights to 400 or 500 â€” NouvelR Bold is the brand voice, lighter weights read as off-brand
- Add decorative borders to PromoCards or content containers â€” separation comes from background color alternation
- Use the semantic colors (Success Green, Error Rose) for decorative purposes â€” they're reserved for form states
- Apply the 56px hero size to anything below the fold â€” hero typography scale is reserved for the carousel
- Create rounded-pill buttons â€” pill shapes are reserved for inputs, never for action elements
- Use flat CSS gradients on UI surfaces â€” the only gradients should be the photographic hero auroras and the text-legibility overlays
...
|------|-------|-------------|
| Mobile Small | â‰¤425px | Single-column, full-width cards, hero text scales to ~32px, stacked CTAs, hamburger nav |
| Mobile | 426â€“640px | Single-column, slightly larger cards, hero text at 32â€“40px |
| Tablet Small | 641â€“768px | 2-column PromoCard grid begins, hero maintains full-width |
| Tablet | 769â€“896px | Full 2-column layout, vehicle range shows 2â€“3 cards |
| Desktop Small | 897â€“1024px | Navigation fully expanded, hero at 56px, 2-up card grid |
| Desktop | 1025â€“1280px | Full layout, 3-up card grid, generous whitespace |
| Large Desktop | 1281â€“1440px | Maximum content width, centered container, hero at full cinematic scale |

### Touch Targets
- All buttons: minimum 46Ã—46px (`minWidth: 46px, minHeight: 46px`) â€” exceeds WCAG AAA 44Ã—44px requirement
...
- **PromoCard grid**: 3-up â†’ 2-up â†’ single-column as viewport narrows
...
1. Focus on ONE component at a time â€” Renault's system has clear component boundaries (PromoCard, VehicleRangeCard, CTA variants)
2. Reference specific color names and hex codes â€” the palette is small but each color has a precise function
3. Use natural language descriptions, not CSS values â€” "sharp zero-radius rectangle" conveys intent better than "border-radius: 0"
4. Describe the desired "feel" alongside specific measurements â€” "assertive automotive energy" communicates the NouvelR Bold heading personality better than "font-weight: 700"
5. Always check whether a section should be light or dark â€” the chessboard alternation is a core pattern
6. Reserve Renault Yellow for ONE button per screen â€” if yellow appears in more than one CTA, the hierarchy collapses
