...
Tesla's website is an exercise in radical subtraction â€” a digital showroom where the product is everything and the interface is almost nothing. The page opens with a full-viewport hero that fills the entire screen with cinematic car photography: three vehicles arranged on polished concrete against a hazy cityscape sky, with a single model name floating above in translucent white type. There are no decorative borders, no gradients, no patterns, no shadows. The UI exists only to provide just enough navigational structure to get out of the way. Every pixel that isn't product imagery is white space, and that restraint is the design system's most powerful statement.

The color philosophy is almost ascetic: a single blue (`#3E6AE1`) for primary calls to action, three shades of dark gray for text hierarchy, and white for everything else. The entire emotional weight is carried by photography â€” sprawling landscape shots, studio-lit vehicle profiles, and atmospheric environmental compositions that stretch edge-to-edge across each viewport-height section. The UI chrome dissolves into the imagery. The navigation bar floats above the hero with no visible background, border, or shadow â€” the TESLA wordmark and five navigation labels simply exist in the space, trusting the content beneath them to provide sufficient contrast.

Typography recently transitioned from Gotham to Universal Sans â€” a custom family split into "Display" for headlines and "Text" for body/UI elements â€” unifying the website, mobile app, and in-car software into a single typographic voice. The Display variant renders hero titles at 40px weight 500, while the Text variant handles everything from navigation (14px/500) to body copy (14px/400). The font carries a geometric precision with slightly humanist terminals that feels engineered rather than designed â€” exactly matching Tesla's brand identity of technology that doesn't need to announce itself. There are no text shadows, no text gradients, no decorative type treatments. Every letterform earns its place through clarity alone.
...
- Single accent color â€” Electric Blue (`#3E6AE1`) â€” used exclusively for primary CTA buttons
...
### Primary
- **Electric Blue** (`#3E6AE1`): Primary CTA button background â€” a confident, mid-saturation blue (rgb 62, 106, 225) that stands alone as the only chromatic color in the entire interface. Used exclusively for "Order Now" and other primary action buttons
- **Pure White** (`#FFFFFF`): Dominant background color for all surfaces, panels, navigation, and secondary button fills â€” the canvas that lets photography breathe

### Secondary & Accent
- **Promo Blue** (`#3E6AE1`): Blue also serves for promotional text ("0% APR Available") displayed over hero imagery in the same hue as the CTA â€” creating a visual link between incentive messaging and action
...
- **Light Ash** (`#F4F4F4`): Subtle alternate surface for section differentiation â€” barely perceptible shift from pure white (rgb 244, 244, 244)
- **Carbon Dark** (`#171A20`): Dark surface color for hero text overlays and potential dark-mode contexts (rgb 23, 26, 32) â€” a warm near-black with a blue undertone
- **Frosted Glass** (`rgba(255, 255, 255, 0.75)`): Semi-transparent white for navigation backdrop-filter effects on scroll

### Neutrals & Text
- **Carbon Dark** (`#171A20`): Primary heading and navigation text â€” the darkest text value (rgb 23, 26, 32), used for model names, nav labels, and hero titles on light backgrounds
- **Graphite** (`#393C41`): Body text and secondary content (rgb 57, 60, 65) â€” the default paragraph color, slightly warmer than pure gray
...
### Font Family
- **Display**: `Universal Sans Display`, -apple-system, Arial, sans-serif â€” used for hero titles and large model names. A geometric sans-serif with precisely engineered proportions, recently replacing Gotham to unify Tesla's digital ecosystem (website, mobile app, vehicle interface)
- **Text/UI**: `Universal Sans Text`, -apple-system, Arial, sans-serif â€” used for navigation, body copy, buttons, and all UI text. Optimized for legibility at smaller sizes with slightly wider proportions than the Display variant
- **No OpenType features** detected â€” typography is completely unembellished
...
| Category Label | 16px (est.) | 500 | â€” | normal | White text labels on category cards ("Sport Sedan") |
...
- **Weight restraint**: Only two weights appear â€” 500 (medium) for headings/UI and 400 (regular) for body. No bold (700), no light (300). The system avoids typographic drama
...
- **Display vs Text split**: The two-variant system (Display for hero, Text for UI) creates subtle optical correction without visible stylistic difference â€” they appear as the same typeface at different sizes
- **No text transforms**: No uppercase text appears in the main navigation or CTAs â€” the lowercase approach reinforces Tesla's understated confidence

## 4. Component Stylings

### Buttons
All buttons use barely-rounded rectangles (4px border-radius) â€” creating a sharp, technical aesthetic that mirrors the precision of the vehicles.

**Primary CTA** â€” The main action button:
...
**Secondary CTA** â€” The alternative action button:
...
**Nav Button** â€” Top navigation items:
...
**Text Link** â€” In-content actions:
...
- No hover animation on the card itself â€” interaction is via the text links beneath
...
- No shadow, no border, no overlay gradient â€” text relies on image darkness for contrast
...
- **Dropdown panel**: Full-width white panel with 3-column vehicle grid + right sidebar text links, no shadow, no border â€” appears seamlessly below the nav
- **Sticky behavior**: `sticky-without-slide` class â€” stays at top without slide-in animation
- **Mobile**: Hamburger collapse pattern
- **No visible separator** between nav and content â€” the nav blends with the hero

### Image Treatment
- **Hero**: Full-viewport (100vh) sections with cinematic photography â€” edge-to-edge, no padding, no margin
...
Tesla uses whitespace as a luxury signal. The generous vertical spacing between sections (each section is a full viewport height) means you can only see one "message" at a time â€” one car, one model name, one CTA pair. This creates a gallery-like browsing experience where each scroll is a deliberate transition, not a continuous feed. White space is not empty â€” it's the frame that elevates each vehicle to the status of art piece.

### Border Radius Scale
| Value | Context |
|-------|---------|
| 0px | Most elements â€” sharp edges are the default |
| 4px | Buttons (primary, secondary, nav items) â€” barely perceptible rounding |
| ~12px | Category cards â€” noticeable but restrained rounding on larger surfaces |
| 50% | Carousel dot indicators â€” perfect circles |

## 6. Depth & Elevation

| Level | Treatment | Use |
|-------|-----------|-----|
| Level 0 (Flat) | No shadow, no border | Default state for all elements â€” cards, panels, buttons at rest |
| Level 1 (Frost) | `rgba(255,255,255,0.75)` backdrop | Navigation bar on scroll â€” frosted glass transparency |
...
3. **Photography-as-depth**: The full-bleed images create their own visual depth through perspective, lighting, and composition â€” making UI shadows redundant

### Decorative Depth
- No gradients, glows, or atmospheric effects on UI elements
- The hero imagery itself provides all visual richness â€” sunset skies, reflected light on car surfaces, ground shadows from studio lighting
- The carousel arrow buttons use a semi-transparent white background to float above the hero imagery without disrupting it

## 7. Do's and Don'ts

### Do
- Let photography dominate every screen â€” the product IS the design
- Use Electric Blue (`#3E6AE1`) exclusively for primary CTAs â€” never for decorative purposes
- Maintain viewport-height sections for major content blocks â€” one message per screen
- Keep typography at weight 400-500 only â€” no bold, no light, no extremes
- Use 4px border-radius for all interactive elements â€” precision over playfulness
- Trust whitespace as a luxury signal â€” never fill available space just because it's empty
- Keep all transitions at 0.33s â€” consistency in motion is as important as consistency in color
- Use transparent PNG vehicle imagery on white backgrounds for product showcases
- Center CTAs horizontally below model names â€” the vertical rhythm is model â†’ subtitle â†’ buttons
- Maintain the Display/Text font split â€” Display for hero-scale text only, Text for everything else

### Don't
- Add shadows to any element â€” elevation through shadow contradicts the flat, gallery aesthetic
- Use more than one chromatic color besides the blue CTA â€” the palette is intentionally monochrome-plus-one
- Apply gradients, patterns, or decorative backgrounds to surfaces â€” white and photography are the only backgrounds
- Use text larger than 40px on the web â€” the typography is deliberately restrained even at hero scale
- Add borders to cards or containers â€” separation is achieved through spacing, not lines
- Use uppercase text transforms â€” Tesla's confidence is expressed through lowercase calm
- Introduce rounded-pill buttons or large border-radii â€” the 4px radius is deliberate and precise
- Override the Universal Sans family with other typefaces â€” cross-platform consistency is a core brand value
- Add hover animations with scale/translate transforms â€” Tesla's interactions are color-only (background and border transitions)
- Clutter the viewport with multiple CTAs â€” every screen should have at most two action buttons
...
### Touch Targets
- Primary CTA buttons: 200px Ã— 40px minimum (well above 44Ã—44px WCAG requirement)
- Nav buttons: minimum 32px height with 4px 16px padding â€” adequate touch targets
...
1. Focus on ONE component at a time â€” Tesla's system is so minimal that each element must be pixel-perfect
2. Reference specific color names and hex codes from this document â€” there are only 6-7 colors in the entire system
3. Use natural language descriptions, not CSS values â€” "barely rounded corners" not "border-radius: 4px"
4. Describe the desired "feel" alongside specific measurements â€” "gallery-like silence between sections" communicates the whitespace philosophy better than "margin-bottom: 100vh"
5. Always verify that photography is doing the emotional heavy-lifting â€” if the UI itself feels "designed," it's too much
